...
8. **生效窗**:兩種到期方式換算正確;未生效不可用,生效即時可用;到期後餘額即時排除;`expire` 交易與 `points.batch.expired` 事件於週期內補上(可訂閱驗證)。
