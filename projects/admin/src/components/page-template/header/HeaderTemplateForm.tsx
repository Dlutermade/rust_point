...
// 頁首模板表單。頁首是外框:沒有生效條件、沒有外框覆寫、預覽不需要疊上下文,
// 所以 props 只留這幾個 —— 不從首頁那份複製一堆用不到的欄位。
export interface HeaderTemplateFormValues {
  name: string
  content: BlockInstance[]
  [key: string]: unknown
}

type HeaderTemplateFormProps = {
  form: FormInstance
  initialValues: HeaderTemplateFormValues
  readOnly?: boolean
...
export function HeaderTemplateForm({
  form,
  initialValues,
  readOnly,
  extraFields,
